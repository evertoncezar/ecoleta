...
function handleComposeMail(){
  MailComposer.composeAsync({
    ...
    recipients: [dataPoint.email],

  });
}

function handleWhatsapp(){
  Linking.openURL(`whatsapp://send?phone=${dataPoint.whatsapp}&text=Tenho interesse sobre coleta de resíduos`);
}
    ...
    <SafeAreaView style={{flex: 1}}>
      <View style={styles.container}>
        <TouchableOpacity onPress={handleNavigateBack}>
          <Icon name="arrow-left" size={30} color="#34cb79"></Icon>
        </TouchableOpacity>

        <Image style={styles.pointImage} source={{ uri: dataPoint.image_url }} />

        <Text style={styles.pointName}>{dataPoint.name} </Text>
        ...
        <View style={styles.address}>
          ...
          <Text style={styles.addressContent}>{dataPoint.city}, {dataPoint.uf}</Text>
        </View>
      </View>
